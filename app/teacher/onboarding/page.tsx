...
export default async function OnboardingPage() {
  ...
  const [courses, lessons, students, staff] = await Promise.all([prisma.course.count({ where: { tenantId } }), prisma.lesson.count({ where: { tenantId } }), prisma.tenantMember.count({ where: { tenantId, role: "STUDENT" } }), prisma.tenantMember.count({ where: { tenantId, role: { in: tenantStaffRoles } } })]);
  const checklist = [
    ["إضافة الشعار والهوية", Boolean(context.membership.tenant.logoUrl), "/teacher/branding"],
    ["إنشاء أول كورس", courses > 0, "/teacher/courses"],
    ["إضافة أول درس", lessons > 0, "/teacher/courses"],
    ["انضمام أول طالب", students > 0, "/teacher/students"],
    ["دعوة عضو للفريق", staff > 1, "/teacher/staff"],
    ["نشر الصفحة العامة", Boolean(context.membership.tenant.settings?.publicPageLive), "/teacher/settings"],
  ] as const;

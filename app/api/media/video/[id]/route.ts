  ...
  try {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      const media = await tx.mediaAsset.update({ where: { id: asset.id }, data: {
        ...state, uploadProgress: state.processingStatus === "READY" ? 100 : Math.max(asset.uploadProgress, video.encodeProgress ?? 0),
      ...
      if (asset.lessonId && state.processingStatus === "READY") {
        await tx.lesson.updateMany({ where: { id: asset.lessonId, tenantId: asset.tenantId }, data: { videoId: null, videoUrl: embedUrl, videoProvider: "BUNNY_STREAM", thumbnailUrl, duration: video.length ? Math.round(video.length) : undefined } });
      }
      return media;
    });

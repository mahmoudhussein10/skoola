...
export function LoginForm({ variant = "default", tenantSlug, portal }: { variant?: "default" | "super-admin"; tenantSlug?: string; portal?: "student" | "teacher" } = {}) {
  ...
  return <form className="authForm" onSubmit={submit} noValidate>
    ...
    {variant === "super-admin" ? <p className="authSwitch">هذا المدخل مخصص لحسابات الإدارة العليا فقط.</p> : resolvedPortal === "teacher" ? <div className="authAccountPrompt managedTeacherAccount"><span>ليس لديك حساب مدرس؟</span><Link className="authCreateAccount" href="/teacher-register">إنشاء حساب مدرس جديد</Link><small>أنشئ منصتك وأكمل بياناتك، ثم ابدأ تجهيز الأكاديمية.</small></div> : tenantSlug ? <div className="authAccountPrompt"><span>طالب جديد في المنصة؟</span><Link className="authCreateAccount" href={`/t/${tenantSlug}/register`}>أنشئ حساب طالب جديد</Link><small>التسجيل يستغرق دقائق ويحفظ تقدمك ونتائجك.</small></div> : <p className="authSwitch">طالب جديد؟ <Link href="/register">اختر أكاديميتك وأنشئ حسابك</Link></p>}
  </form>;
}

...
import { ArrowLeft, BarChart3, BookOpenCheck, Check, ChevronLeft, GraduationCap, Layers3, LockKeyhole, Palette, Play, ShieldCheck, Sparkles, Users, Zap } from "lucide-react";
import { prisma } from "../lib/prisma";
import { AnimatedNumber, Reveal } from "./skoola-motion";
import { Brand } from "./ui";

// The marketing homepage does not depend on request-specific state. ISR avoids
// re-running three aggregate database queries for every visitor/click.
...
const plans = [
  { name: "Starter", price: "ابدأ الآن", note: "لبداية أكاديمية منظمة ببياناتك الحقيقية", features: ["مساحة مدرس مستقلة", "هوية خاصة بمنصتك", "إدارة الكورسات والطلاب", "بدون بيانات تجريبية"] },
...
export default async function PlatformHome() {
  ...
  return <main className="skoolaHome">
    <header className="skoolaHeader"><nav className="skoolaWrap skoolaNav"><Brand /><div className="skoolaNavLinks"><Link href="#features">المميزات</Link><Link href="#solutions">الحلول</Link><Link href="#pricing">الأسعار</Link><Link href="#faq">الأسئلة</Link></div><div className="skoolaNavActions"><Link href="/login">تسجيل الدخول</Link><Link className="skoolaBtn small" href="/teacher-register">ابدأ مجانًا <ArrowLeft size={16} /></Link></div></nav></header>

    <section className="skoolaHero skoolaWrap">
      <div className="skoolaBlob blobOne" /><div className="skoolaBlob blobTwo" />
      <Reveal className="skoolaHeroCopy"><span className="skoolaPill"><Sparkles size={15} /> منصة التعليم العربية الجديدة</span><h1>علّم بطريقتك.<br /><span>وانمُ بلا حدود.</span></h1><p>Skoola تمنحك كل ما تحتاجه لبناء أكاديمية رقمية احترافية، إدارة طلابك، نشر محتواك، وقياس النمو من مكان واحد.</p><div className="skoolaHeroActions"><Link className="skoolaBtn" href="/teacher-register">أنشئ منصتك مجانًا <ArrowLeft size={19} /></Link><Link className="skoolaVideoBtn" href="/teacher-register"><i><Play size={17} fill="currentColor" /></i> أنشئ أكاديميتك</Link></div><div className="skoolaProof"><span><Check size={15} /> بدون بطاقة بنكية</span><span><Check size={15} /> إعداد خلال دقائق</span><span><Check size={15} /> عربي وRTL</span></div></Reveal>
    ...
    <section className="skoolaSection skoolaWrap" id="solutions"><Reveal className="skoolaSectionHead"><span>لكل رحلة تعليمية</span><h2>مدخل واضح لكل شخص.<br />وتجربة مصممة لدوره.</h2><p>من أول تسجيل المدرس حتى متابعة آخر طالب، كل خطوة متصلة وسهلة.</p></Reveal><div className="solutionGrid"><Reveal className="solutionCard teacherSolution"><div className="solutionIcon"><GraduationCap /></div><small>للمدرسين والأكاديميات</small><h3>حوّل خبرتك إلى أكاديمية متكاملة</h3><p>أنشئ المنصة، أضف المحتوى، كوّن فريقك، وتابع أداء طلابك من لوحة واحدة.</p><Link href="/teacher-register">ابدأ كمدرس <ChevronLeft size={17}/></Link></Reveal><Reveal className="solutionCard studentSolution" delay={.08}><div className="solutionIcon"><BookOpenCheck /></div><small>للطلاب</small><h3>تعلّم بوضوح وواصل تقدمك</h3><p>كل كورساتك ونتائجك وواجباتك وإشعاراتك في مساحة تعليمية هادئة.</p><Link href="/register">أنشئ حساب طالب <ChevronLeft size={17}/></Link></Reveal><Reveal className="solutionCard adminSolution" delay={.16}><div className="solutionIcon"><LockKeyhole /></div><small>للإدارة العليا</small><h3>تحكم مؤسسي بلا فوضى</h3><p>راقب النمو والمنصات والأمان والإعلانات من مركز قيادة محمي.</p><Link href="/super-admin/login">دخول Super Admin <ChevronLeft size={17}/></Link></Reveal></div></section>
    ...
    <section className="skoolaCta"><div className="skoolaWrap"><div><span>جاهز للنمو؟</span><h2>أطلق أكاديميتك على Skoola اليوم.</h2><p>ابدأ مجانًا، وأضف أول كورس وأول طالب خلال دقائق.</p></div><Link className="skoolaBtn light" href="/teacher-register">أنشئ منصتك <ArrowLeft size={19}/></Link></div></section>
    <footer className="skoolaFooter"><div className="skoolaWrap footerGrid"><div><Brand inverse/><p>Teach · Learn · Grow<br/>منصة تعليم حديثة تنمو معك.</p></div><div><b>المنتج</b><Link href="#features">المميزات</Link><Link href="#pricing">الأسعار</Link><Link href="/teacher-register">للمدرسين</Link></div><div><b>الحساب</b><Link href="/login">تسجيل الدخول</Link><Link href="/register">حساب طالب</Link><Link href="/forgot-password">استعادة الحساب</Link></div><div><b>الإدارة</b><Link href="/super-admin/login">Super Admin</Link><Link href="/teacher-register">إنشاء منصة</Link></div></div><div className="skoolaWrap footerBottom"><span>© {new Date().getFullYear()} Skoola. جميع الحقوق محفوظة.</span><span>Teach · Learn · Grow</span></div></footer>
  </main>;
}
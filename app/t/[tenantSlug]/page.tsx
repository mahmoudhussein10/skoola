...
export default async function TenantPublicPage({ params }: { params: Promise<{ tenantSlug: string }> }) {
  ...
  const style = {
    ...
    fontFamily: `${theme?.fontFamily ?? "Tajawal"}, "Segoe UI", Tahoma, Arial, sans-serif`,
  } as CSSProperties;

  const primaryHref = isLoggedInStudent ? "/dashboard" : "/t/" + tenant.slug + "/register";
  return <main className="tenantPublic tenantShowcase" style={style}>
    <nav className="tenantNav wrap"><Link className="brand tenantBrand" href={"/t/" + tenant.slug}><b className="tenantLogo">{tenant.logoUrl ? <Image src={tenant.logoUrl} alt="" width={48} height={48} /> : tenant.name.slice(0, 1)}</b><span>{tenant.settings?.platformName ?? tenant.name}<small>{tenant.subject ?? "منصة تعليمية"}</small></span></Link><div className="tenantNavLinks"><Link href="#courses">الكورسات</Link><Link href="#why">لماذا منصتنا؟</Link>{isLoggedInStudent ? <Link className="btn tenantPrimary" href="/dashboard">لوحتي التعليمية <ArrowLeft size={16} /></Link> : <><Link href={"/t/" + tenant.slug + "/login"}>تسجيل الدخول</Link><Link className="btn tenantPrimary" href={"/t/" + tenant.slug + "/register"}>ابدأ الآن <ArrowLeft size={16} /></Link></>}</div></nav>
    <section className="tenantHero wrap"><div className="tenantHeroCopy"><span className="tenantEyebrow"><Sparkles size={16} /> تعلّم مع {tenant.name}</span><h1>{tenant.settings?.heroTitle ?? "فهم أعمق. نتائج أقوى. رحلة تعليم واضحة."}</h1><p>{tenant.settings?.description ?? tenant.description ?? "محتوى منظم، شرح واضح، ومتابعة حقيقية تساعدك تتقدم بثقة خطوة بخطوة."}</p><div className="tenantHeroActions"><Link className="btn tenantPrimary" href={primaryHref}>{isLoggedInStudent ? "اذهب إلى لوحتي" : "أنشئ حسابك مجانًا"} <ArrowLeft size={18} /></Link><Link className="tenantTextButton" href="#courses">استكشف الكورسات</Link></div><div className="tenantHeroProof"><span><CheckCircle2 size={16} /> محتوى منظم</span><span><CheckCircle2 size={16} /> متابعة للتقدم</span><span><CheckCircle2 size={16} /> تجربة آمنة</span></div></div>
      <div className="tenantTeacher"><span className="teacherRing one" /><span className="teacherRing two" /><span className="tenantHeroAura" /><div className="tenantFloatingBadge badgeTop"><BookOpenCheck size={21} /><span><b>{courses.length.toLocaleString("en-US")} كورس</b><small>متاح على المنصة</small></span></div><Image src={theme?.loginCoverUrl || "/hero.png"} alt={tenant.name} width={700} height={600} priority /><div className="tenantFloatingBadge badgeBottom"><span className="liveDot" /><span><b>تعلّم بطريقتك</b><small>من أي جهاز وفي أي وقت</small></span></div></div>
    </section>
    <section className="tenantTrustBar"><div className="wrap"><div><b>{courses.length.toLocaleString("en-US")}</b><span>كورس منشور</span></div><div><b>{totalStudents.toLocaleString("en-US")}</b><span>اشتراك تعليمي</span></div><div><b>RTL</b><span>واجهة عربية متكاملة</span></div><div><b>آمن</b><span>بيانات كل منصة معزولة</span></div></div></section>
    <section id="courses" className="tenantCourses wrap"><div className="tenantSectionHead"><div><span>ابدأ رحلتك</span><h2>كورسات مصممة لتفهم وتتقدم</h2><p>اختر الكورس المناسب لصفك، واعرف تفاصيله قبل الاشتراك.</p></div>{isLoggedInStudent ? <Link href="/dashboard">عرض كورساتي <ArrowLeft size={17} /></Link> : null}</div>{courses.length ? <div className="tenantCourseGrid">{courses.map((course, index) => <article id={"course-" + course.id} className={`tenantCourseCard tone-${index % 3}`} key={course.id}><div className="tenantCourseArt">{course.thumbnailUrl ? <CourseThumbnail src={course.thumbnailUrl} alt={course.title} /> : <><span>{course.subject}</span><b>{course.subject.slice(0, 2)}</b><i /><em /></>}</div><div className="tenantCourseBody"><div className="courseMeta"><span>{gradeLabels[course.grade]}</span><small><Users size={13} /> {course._count.enrollments.toLocaleString("en-US")} طالب</small></div><h3>{course.title}</h3><p>{course.description}</p><div className="tenantCourseFooter"><span><small>سعر الكورس</small><b>{Number(course.price) === 0 ? "مجاني" : Number(course.price).toLocaleString("en-US") + " ج.م"}</b></span>{enrolledCourseIds.has(course.id) ? <Link className="tenantCourseAction enrolled" href={"/course?courseId=" + course.id}>متابعة الكورس <ArrowLeft size={16} /></Link> : <Link className="tenantCourseAction" href={"/t/" + tenant.slug + "/courses/" + course.slug}>تفاصيل الكورس <ArrowLeft size={16} /></Link>}</div></div></article>)}</div> : <div className="emptyState tenantCourseEmpty"><span>✦</span><h3>الكورسات قيد التجهيز</h3><p>سيظهر محتوى المدرس هنا فور نشر أول كورس.</p></div>}</section>
    <section id="why" className="tenantValueBand"><div className="wrap"><div className="tenantSectionHead light"><div><span>تجربة متكاملة</span><h2>كل ما تحتاجه للتعلّم بثقة</h2><p>المنصة تربط المحتوى والتقدم والنتائج في مكان واحد هادئ وواضح.</p></div></div><div className="tenantValueGrid"><article><i><BookOpenCheck /></i><h3>محتوى مرتب</h3><p>دروس ووحدات مرتبة لتعرف دائمًا أين أنت وما الخطوة التالية.</p></article><article><i><GraduationCap /></i><h3>متابعة حقيقية</h3><p>تقدمك ونتائجك محفوظة داخل حسابك وتظهر لك بوضوح.</p></article><article><i><ShieldCheck /></i><h3>خصوصية وأمان</h3><p>حسابك مرتبط بمنصة مدرسك، وبيانات كل منصة معزولة عن غيرها.</p></article></div></div></section>
    <section className="tenantFinalCta"><div className="wrap"><div><span>جاهز تبدأ؟</span><h2>{isLoggedInStudent ? "ارجع لمساحتك وكمّل تقدمك." : "أنشئ حسابك وانضم إلى رحلة التعلّم."}</h2></div><Link className="btn tenantPrimary" href={primaryHref}>{isLoggedInStudent ? "افتح لوحتي" : "ابدأ الآن"} <ArrowLeft size={18} /></Link></div></section>

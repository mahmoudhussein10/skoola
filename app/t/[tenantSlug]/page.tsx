...
export default async function TenantPublicPage({ params }: { params: Promise<{ tenantSlug: string }> }) {
  ...
  const teacherVisual = theme?.teacherPortraitUrl || theme?.heroImageUrl || theme?.loginCoverUrl || null;
  return <main className={`tenantPublic tenantShowcase tenantHeroLayout-${(theme?.heroLayout ?? "SPLIT").toLowerCase()}`} style={style}>
    ...
    <section className="tenantHero wrap" style={sectionStyle("HERO")}><div className="tenantHeroCopy tenantReveal"><span className="tenantEyebrow"><Sparkles size={16} /> تعلّم مع {tenant.name}</span><h1>{theme?.heroTitle ?? tenant.settings?.heroTitle ?? "فهم أعمق. نتائج أقوى. رحلة تعليم واضحة."}</h1><p>{theme?.heroSubtitle ?? tenant.settings?.description ?? tenant.description ?? "محتوى منظم، شرح واضح، ومتابعة حقيقية تساعدك تتقدم بثقة خطوة بخطوة."}</p><div className="tenantHeroActions"><Link className="btn tenantPrimary" href={primaryHref}>{isLoggedInStudent ? "اذهب إلى لوحتي" : (theme?.heroCtaLabel ?? "أنشئ حسابك مجانًا")} <ArrowLeft size={18} /></Link><Link className="tenantTextButton" href="#courses">{theme?.heroSecondaryLabel ?? "استكشف الكورسات"}</Link></div><div className="tenantHeroProof"><span><CheckCircle2 size={16} /> محتوى منظم</span><span><CheckCircle2 size={16} /> متابعة للتقدم</span><span><CheckCircle2 size={16} /> تجربة آمنة</span></div><div className="tenantHeroLiveStats" aria-label="ملخص المنصة"><span><b>{courses.length.toLocaleString("en-US")}</b><small>كورس متاح</small></span><span><b>{totalStudents.toLocaleString("en-US")}</b><small>اشتراك تعليمي</small></span><span><b>24/7</b><small>تعلّم في أي وقت</small></span></div></div>
      <div className={`tenantTeacher tenantReveal tenantRevealDelay${teacherVisual ? "" : " tenantTeacherEmpty"}`}><span className="teacherRing one" /><span className="teacherRing two" /><span className="tenantHeroAura" /><div className="tenantFloatingBadge badgeTop"><BookOpenCheck size={21} /><span><b>{courses.length.toLocaleString("en-US")} كورس</b><small>متاح على المنصة</small></span></div>{teacherVisual ? <Image src={teacherVisual} alt={tenant.name} width={700} height={600} priority /> : <div className="tenantHeroPlaceholder" aria-label={`واجهة منصة ${tenant.name}`}><span><GraduationCap size={52} /></span><small>منصة تعليمية متكاملة</small><strong>{tenant.settings?.platformName || tenant.name}</strong><p>ابدأ التعلّم بخطوات واضحة ومنظمة</p></div>}<div className="tenantFloatingBadge badgeBottom"><span className="liveDot" /><span><b>تعلّم بطريقتك</b><small>من أي جهاز وفي أي وقت</small></span></div></div>
    </section>
